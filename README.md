...
| Database | MariaDB |
| Auth | JWT |
| Security | bcrypt, Zero Trust model |
...
웨어러블 기기에서 수집되는 바이탈 데이터는 기기마다 단위와 정확도가 다르고,  
그대로 시각화할 경우 일관된 해석이 어렵다.</br>

VitaCore는 이러한 문제를 해결하기 위해 실측 데이터를 그대로 사용하지 않고,  
생리학적 시뮬레이션을 통해 표준화된 상태로 변환한 후 사용자에게 제공하도록 설계되었다.
